...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['email'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM tbl_admin WHERE email = :email');
    ...
    if ($user) {
        ...
        $stmt = $pdo->prepare('UPDATE tbl_admin SET reset_token = :token, token_expiry = :expiry WHERE email = :email');
        $stmt->execute([
            'token' => $token,
            'expiry' => $expiry,
            'email' => $email
        ...
        try {
            ...
            // Content
            $reset_link = "https://ims-pnc.site/admin_reset_password.php?token=" . $token;
            $mail->isHTML(true);
            $mail->Subject = 'Password Reset Request';
            $mail->Body    = "Good day Admin!, <br><br>We've received a request to reset your PIN. If this was you, please confirm and continue by clicking the link below: <br>
