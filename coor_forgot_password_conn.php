...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['email'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM tbl_coordinators WHERE email = :email');
    ...
    if ($user) {
        ...
        $stmt = $pdo->prepare('UPDATE tbl_coordinators SET reset_token = :token, token_expiry = :expiry WHERE email = :email');
        $stmt->execute([
            'token' => $token,
            'expiry' => $expiry,
            'email' => $email
        ...
        try {
            ...
            // Content
            $reset_link = "https://ims-pnc.site/coor_reset_password.php?token=" . $token;
            $mail->isHTML(true);
    ...
    header("Location: forgot_password_coor.php?msg=" . urlencode($msg));
    exit;
}
?>
