              ...
              AND r.status = 'Pending';
                ");
                        ...
                        foreach ($results as $row):
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#fileModal<?php echo $row['id']; ?>">
                            ...
                            <div class="modal fade" id="fileModal<?php echo $row['id']; ?>" tabindex="-1"
                                aria-labelledby="fileModalLabel<?php echo $row['id']; ?>" aria-hidden="true">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            if (file_exists($filePath)): ?>
                                                <?php if (in_array($fileExtension, ['pdf'])): ?>
                                                    <embed src="<?php echo $filePath; ?>" type="application/pdf" width="100%"
                                                        height="500px" />
                                                <?php elseif (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                                    <img src="<?php echo $filePath; ?>" alt="Image preview"
                                                ...
                                                <?php elseif (in_array($fileExtension, ['doc', 'docx'])): ?>
                                                    <p>Microsoft Word Document: <a href="<?php echo $filePath; ?>"
                                                            target="_blank">Download Document</a></p>
                                                <?php elseif (in_array($fileExtension, ['xls', 'xlsx'])): ?>
                                                    <p>Microsoft Excel Spreadsheet: <a href="<?php echo $filePath; ?>"
                                                            target="_blank">Download Spreadsheet</a></p>
                                                <?php else: ?>
                                        ...
                                        <div class="modal-footer">
                                            <?php if ($row['status'] == "Pending"): ?>
                                                ...
                                                <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#cancelModal<?php echo $row['id']; ?>">Reject</button>
                                            <?php endif; ?>
                            ...
                            <div class="modal fade" id="cancelModal<?php echo $row['id']; ?>" tabindex="-1"
                                aria-labelledby="cancelModalLabel<?php echo $row['id']; ?>" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="cancelModalLabel<?php echo $row['id']; ?>">Cancel
                                                Requirement</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form method="post" action="file_review.php">
                                                ...
                                                <input type="hidden" name="student_id"
                                                    value="<?php echo htmlspecialchars($row['student_id']); ?>">
                                                <div class="mb-3">
                                                    <label for="cancel_reason<?php echo $row['id']; ?>"
                                                        class="form-label">Cancellation Reason</label>
                                                    <textarea id="cancel_reason<?php echo $row['id']; ?>"
                                                        name="cancel_reason" class="form-control" required></textarea>
                                                </div>
                                                <button type="submit" name="cancel" class="btn btn-danger">Submit
                                                    Cancellation</button>
                                            </form>
                                        </div>
                                    </div>
