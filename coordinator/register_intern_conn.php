...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($count > 0) {
        ...
        $address = $unit_block . ', ' . ($subdivision ? $subdivision . ', ' : '') . $barangay . ', ' . $city . ', ' . $province;
        
        $coordinator_id = $_POST['coordinator_id'];
        $status = $_POST['status'];

        $stmt = $pdo->prepare("INSERT INTO tbl_users (student_id, pin, firstname, lastname, email, phone, address, program_id,company_id, coordinator_id, status) 
        VALUES (:student_id, :pin, :firstname, :lastname, :email, :phone, :address, :program_id, :company_id, :coordinator_id, :status)");
        $stmt->bindParam(':student_id', $student_id);
